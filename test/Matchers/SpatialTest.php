<?php

namespace ZxcvbnPhp\Test\Matchers;

use ZxcvbnPhp\Matchers\Match;
use ZxcvbnPhp\Matchers\SpatialMatch;

/**
 * @covers \ZxcvbnPhp\Matchers\SpatialMatch
 */
class SpatialTest extends AbstractMatchTest
{
    public function shortPatternDataProvider()
    {
        return [
            [''],
            ['/'],
            ['qw'],
            ['*/'],
        ];
    }

    /**
     * @dataProvider shortPatternDataProvider
     * @param $password
     */
    public function testShortPatterns($password)
    {
        $this->assertEquals(
            [],
            SpatialMatch::match($password),
            "doesn't match 1- and 2-character spatial patterns"
        );
    }

    public function testNoPattern()
    {
        $this->assertEquals(
            [],
            SpatialMatch::match('qzpm'),
            "doesn't match non-pattern"
        );
    }

    public function surroundedPatternProvider()
    {
        return [
            ['6tfGHJ', 'qwerty'],

            ['-tfGHJ', 'azerty'],
        ];
    }

    /**
     * @dataProvider surroundedPatternProvider
     * @param string $pattern
     * @param string $graph
     */
    public function testSurroundedPattern($pattern, $graph)
    {
        $password = "rz!{$pattern}%z";

        // for testing, make a subgraph that contains a single keyboard
        $graphs = [$graph => SpatialMatch::getAdjacencyGraphs()[$graph]];

        $this->checkMatches(
            "matches against spatial patterns surrounded by non-spatial patterns",
            SpatialMatch::match($password, [], $graphs),
            'spatial',
            [$pattern],
            [[3, 8]],
            [
                'graph' => [$graph],
                'turns' => [2],
                'shiftedCount' => [3],
            ]
        );
    }

    public function spatialDataProvider()
    {
        return [
            ['12345',        'qwerty',     1, 0],
            ['@WSX',         'qwerty',     1, 4],
            ['6tfGHJ',       'qwerty',     2, 3],
            ['hGFd',         'qwerty',     1, 2],
            ['/;p09876yhn',  'qwerty',     3, 0],
            ['Xdr%',         'qwerty',     1, 2],
            ['12345',        'azerty',     1, 5],
            ['-tfGHJ',       'azerty',     2, 3],
            ['159-',         'keypad',     1, 0],
            ['*84',          'keypad',     1, 0],
            ['/8520',        'keypad',     1, 0],
            ['369',          'keypad',     1, 0],
            ['/963.',        'mac_keypad', 1, 0],
            ['*-632.0214',   'mac_keypad', 9, 0],
            ['aoEP%yIxkjq:', 'dvorak',     4, 5],
            [';qoaOQ:Aoq;a', 'dvorak',    11, 4],
        ];
    }

    /**
     * @dataProvider spatialDataProvider
     * @param $password
     * @param $keyboard
     * @param $turns
     * @param $shifts
     */
    public function testSpatialPatterns($password, $keyboard, $turns, $shifts)
    {
        $graphs = [$keyboard => SpatialMatch::getAdjacencyGraphs()[$keyboard]];

        $this->checkMatches(
            "matches '$password' as a $keyboard pattern",
            SpatialMatch::match($password, [], $graphs),
            'spatial',
            [$password],
            [[0, strlen($password) - 1]],
            [
                'graph' => [$keyboard],
                'turns' => [$turns],
                'shiftedCount' => [$shifts],
            ]
        );
    }

    public function testShiftedCountForMultipleMatches()
    {
        $password = "!QAZ1qaz";

        $this->checkMatches(
            "shifted count is correct for two matches in a row",
            SpatialMatch::match($password),
            'spatial',
            ['!QAZ', 'QAZ', '1qaz', 'qaz'],
            [[0, 3], [1, 3], [4, 7], [5, 7]],
            [
                'graph' => ['qwerty', 'azerty', 'qwerty', 'azerty'],
                'turns' => [1, 2, 1, 2],
                'shiftedCount' => [4, 3, 0, 0],
            ]
        );
    }

    protected function getBaseGuessCount($token)
    {
        // KEYBOARD_STARTING_POSITIONS * KEYBOARD_AVERAGE_DEGREE * (length - 1)
        // - 1 term because: not counting spatial patterns of length 1
        // eg for length==6, multiplier is 5 for needing to try len2,len3,..,len6
        return SpatialMatch::KEYBOARD_STARTING_POSITION
            * SpatialMatch::KEYBOARD_AVERAGE_DEGREES
            * (strlen($token) - 1);
    }

    public function guessesBasicProvider()
    {
        return [
            ['zxcvbn', 'qwerty'],

            ['wxcvbn', 'azerty'],
        ];
    }

    /**
     * @dataProvider guessesBasicProvider
     * @param $token
     * @param $graph
     */
    public function testGuessesBasic($token, $graph)
    {
        $match = new SpatialMatch($token, 0, strlen($token) - 1, $token, [
            'graph' => $graph,
            'turns' => 1,
            'shifted_count' => 0,
        ]);

        $this->assertEquals(
            $this->getBaseGuessCount($token),
            $match->getGuesses(),
            "with no turns or shifts, guesses is starts * degree * (len-1)"
        );
    }

    public function guessesShiftedProvider()
    {
        return [
            ['ZxCvbn', 'qwerty'],

            ['WxCvbn', 'azerty'],
        ];
    }

    /**
     * @dataProvider guessesShiftedProvider
     * @param $token
     * @param $graph
     */
    public function testGuessesShifted($token, $graph)
    {
        $match = new SpatialMatch($token, 0, strlen($token) - 1, $token, [
            'graph' => $graph,
            'turns' => 1,
            'shifted_count' => 2,
        ]);

        $this->assertEquals(
            $this->getBaseGuessCount($token) * (Match::binom(6, 2) + Match::binom(6, 1)),
            $match->getGuesses(),
            "guesses is added for shifted keys ($graph), similar to capitals in dictionary matching"
        );
    }

    public function guessesEverythingShiftedProvider()
    {
        return [
            ['ZXCVBN', 'qwerty'],

            ['WXCVBN', 'azerty'],
        ];
    }
    /**
     * @dataProvider guessesEverythingShiftedProvider
     * @param $token
     * @param $graph
     */
    public function testGuessesEverythingShifted($token, $graph)
    {
        $match = new SpatialMatch($token, 0, strlen($token) - 1, $token, [
            'graph' => $graph,
            'turns' => 1,
            'shifted_count' => 6,
        ]);

        $this->assertEquals(
            $this->getBaseGuessCount($token) * 2,
            $match->getGuesses(),
            "when everything is shifted, guesses are double"
        );
    }

    public function complexGuessProvider()
    {
        return [
            // qwerty
            ['6yhgf',        2, 19596.255319547865, 'qwerty'],
            ['asde3w',       3, 203315.1579961936,  'qwerty'],
            ['zxcft6yh',     3, 558460.6174911747,  'qwerty'],
            ['xcvgy7uj',     3, 558460.6174911747,  'qwerty'],
            ['ertghjm,.',    5, 30160744.32861352,  'qwerty'],
            ['qwerfdsazxcv', 5, 175281377.64553562, 'qwerty'],

            // azerty
            ['-yhgf',        2, 19596.255319547865, 'azerty'],
            ['asde"z',       3, 203315.1579961936,  'azerty'],
            ['wxcft-yh',     3, 558460.6174911747,  'azerty'],
            ['xcvgyèuj',     3, 558460.6174911747,  'azerty'],
            ['ertghj,;:',    5, 30160744.32861352,  'azerty'],
            ['azerfdsqwxcv', 5, 175281377.64553562, 'azerty'],
        ];
    }

    /**
     * @dataProvider complexGuessProvider
     * @param string $token
     * @param int $turns
     * @param float $expected
     */
    public function testGuessesComplexCase($token, $turns, $expected, $graph)
    {
        $match = new SpatialMatch($token, 0, strlen($token) - 1, $token, [
            'graph' => $graph,
            'turns' => $turns,
            'shifted_count' => 0,
        ]);

        $this->assertEquals(
            $expected,
            $match->getGuesses(),
            "spatial guesses accounts for turn positions, directions and starting keys"
        );
    }

    public function feedbackStraightLineProvider()
    {
        return [
            ['dfghjk', 'qwerty'],
            ['zxcvbn', 'qwerty'],

            ['dfghjk', 'azerty'],
            ['wxcvbn', 'azerty'],
        ];
    }

    /**
     * @dataProvider feedbackStraightLineProvider
     * @param string $token
     * @param string $graph
     */
    public function testFeedbackStraightLine($token, $graph)
    {
        $match = new SpatialMatch($token, 0, strlen($token) - 1, $token, [
            'graph' => $graph,
            'turns' => 1,
            'shifted_count' => 0,
        ]);
        $feedback = $match->getFeedback(true);

        $this->assertEquals(
            'Straight rows of keys are easy to guess',
            $feedback['warning'],
            "spatial match in straight line gives correct warning"
        );
        $this->assertContains(
            'Use a longer keyboard pattern with more turns',
            $feedback['suggestions'],
            "spatial match in straight line gives correct suggestion"
        );
    }

    public function feedbackWithTurnsProvider()
    {
        return [
            ['xcvgy789', 'qwerty'],

            ['xcvgyè_ç', 'azerty'],
        ];
    }

    /**
     * @dataProvider feedbackWithTurnsProvider
     * @param string $token
     * @param string $graph
     */
    public function testFeedbackWithTurns($token, $graph)
    {
        $match = new SpatialMatch($token, 0, strlen($token) - 1, $token, [
            'graph' => $graph,
            'turns' => 3,
            'shifted_count' => 0,
        ]);
        $feedback = $match->getFeedback(true);

        $this->assertEquals(
            'Short keyboard patterns are easy to guess',
            $feedback['warning'],
            "spatial match with turns gives correct warning"
        );
        $this->assertContains(
            'Use a longer keyboard pattern with more turns',
            $feedback['suggestions'],
            "spatial match with turns gives correct suggestion"
        );
    }
}
